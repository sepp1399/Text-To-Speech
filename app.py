...
class StreamingTTSService:
    ...
    def load(self) -> None:
        ...
        logging.info(f"Using device: {device_map}, torch_dtype: {load_dtype}, attn_implementation: {attn_impl_primary}")
        ...
        try:
            ...
            if attn_impl_primary == 'flash_attention_2':
                logging.warning("Error loading the model. Trying to use SDPA. However, note that only flash_attention_2 has been fully tested.")
                self.model = VibeVoiceStreamingForConditionalGenerationInference.from_pretrained(
                    self.model_path,
                    torch_dtype=load_dtype,
    ...
    def _load_voice_presets(self) -> Dict[str, Path]:
        voices_dir = BASE / "vibevoice" / "voices" / "streaming_model"
    ...
    def _ensure_voice_cached(self, key: str) -> Tuple[object, Path, str]:
        ...
        if key not in self._voice_cache:
            ...
            logging.info(f"[startup] Loading voice preset {key} from {preset_path}")
...
async def lifespan(app: FastAPI):
    ...
    logging.info("[startup] Model ready.")
    
    yield
    
    # Shutdown (se necessario cleanup)
    logging.info("[shutdown] Cleaning up...")

# -----------------------------
# FastAPI App
# -----------------------------
app = FastAPI(title="VibeVoice TTS API", lifespan=lifespan)
...
async def websocket_stream(ws: WebSocket) -> None:
    ...
    logging.info(f"Client connected, text={text!r}")
    ...
    if lock.locked():
        ...
        logging.warning("Please wait for the other requests to complete.")
        try:
